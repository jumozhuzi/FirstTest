...
#     cur_iv=np.mean(getIVArr(opttype_ts,np.ones(opttype_ts.shape)*snp.last,k_ts,np.ones(opttype_ts.shape)*ttm_days/annual_coeff,optlast_ts,1))
#     # idx=snp.datetime
...
def getLatestIV(underlyingCode):
    ...
    for col in moneyness:
        ...
        options_contracts=rqd.options.get_contracts(underlyingCode,strike=snp.last*col,trading_date=cur_trd_date)
        if col>1:
           options_contracts=[options_contracts[0]]
        elif col<1:
           options_contracts=[options_contracts[-1]]
        else:
           pass
        ...
        ttm_days=intr_hours+stm.session_state.trading_list.index(pd.to_datetime(option_instruments[0].maturity_date).date())-stm.session_state.trading_list.index(cur_trd_date)
        ...
        cur_iv=np.mean(getIVArr(np.array(opttype_ts*2),np.ones(len(opttype_ts)*2)*snp.last
                                ,np.array(k_ts*2)
                                ,np.ones(len(opttype_ts)*2)*ttm_days/annual_coeff,np.array(optasks_ts+optbids_ts),1))
...
#----------------------------------------------------------------------------------------------------
asset_lists="AU2412,AU2502,AG2412,AG2502,CF501,CF503,CF505,MA501,MA505,EG2501,EB2501,V2501,PP2501,L2501,SH501,AL2412"
